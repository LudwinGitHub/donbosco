...
export default function BadgeChip({ type }: { type: BadgeType }) {
  ...
  return (
    ...
    >
      ...
      {/* Tooltip */}
      {open && (
        <span className="pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 -translate-x-1/2 whitespace-nowrap rounded-lg border border-zinc-700/60 bg-zinc-900 px-2.5 py-1.5 text-[11px] font-semibold text-white shadow-lg dark:border-white/10 dark:bg-zinc-800">
          {label}
          <span className="absolute left-1/2 top-full -translate-x-1/2 border-4 border-transparent border-t-zinc-900 dark:border-t-zinc-800" />
        </span>
      )}
    </span>
  )
